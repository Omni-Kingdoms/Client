...
export default function DungeonListMagic({ id, disableLoading }: Props) {
  ...
  const handleDungeon = useCallback(async () => {
    const dungeon = await contract.read.getMagicMonster([id]);

    const blockTimestamp = await contract.read.getMagicMonsterCooldown([
          ...
          <Tooltip title="Mana">
            <div className="flex">
              <Image
                ...
                alt="level"
              />
              <p className="mt-2 text-red-600">{Number(dungeon?.cost!)}</p>
            </div>
          </Tooltip>
        </div>
